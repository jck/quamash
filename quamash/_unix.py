...
class _Selector(selectors.BaseSelector):
	...
	def register(self, fileobj, events, data=None):
		...
		if events & EVENT_READ:
			notifier = self.QtCore.QSocketNotifier(key.fd, self.QtCore.QSocketNotifier.Read)
			notifier.activated.connect(self.__on_read_activated)
			self.__read_notifiers[key.fd] = notifier
		if events & EVENT_WRITE:
			notifier = self.QtCore.QSocketNotifier(key.fd, self.QtCore.QSocketNotifier.Write)
